...
def load_sequences(specs, max_sequences=0):
        ...
        for fname in files:
            with np.load(os.path.join(directory, fname)) as data:
                ...
                for spec, key in fields:
                    seq["probs"][spec] = np.asarray(data[key]).astype(np.float32)
            loaded.append(seq)
...
def main(argv=None):
    ...
    sequences, names = load_sequences(args.dump_dir, args.max_sequences)
    ...
    if args.out:
        ...
        with open(args.out, "w", encoding="utf-8") as stream:
            json.dump({"args": vars(args), "readouts": names, "result": result}, stream, indent=2, ensure_ascii=False)
        print("\n报告:", args.out)
    return result
