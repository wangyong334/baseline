...
- 训练加速 A（09-17）：服务器等价核对通过、计时完成；待用加速选项重训 seed37，以实际 epoch_seconds 确认
- 同口径能耗统计：基线 K5 权重在服务器上的路径待确认（`log/baseline_k5_repolr_seed37/` 不存在），之后跑 `baseline_energy.py` 与 `stream_energy.py`
...
| 训练加速 A | GPU 构造输入（事件常驻显存、查表归一化，与 numpy 输入逐位相同）；片段内逐层时间并行 `forward_chunk`（卷积按 T 个窗口批量算，只有膜电位按时间递推）；训练子集评估可隔轮 | 训练 7–9 h、GPU 利用率 10%；此前预估快 3–4 倍，实测好于预估 | 服务器核对通过（float64 零误差，默认设置下验证集 IoU 不变）；训练 6.5 倍、验证 11 倍，估算每轮 566 → 75 s；待重训确认。`--mode eval` 仍走原实现 |
...
V1 单卡训练 50 轮约 6.6–7.6 小时。加速 A（layer + gpu，训练子集每 5 轮）计时估算约 1 小时，待重训实测。
