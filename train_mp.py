...
def linear_epoch_lr(epoch, epochs, start_lr, end_lr):
    """Epoch 0 uses start_lr; epoch epochs-1 uses end_lr (inclusive)."""
    if epochs < 2:
        raise ValueError("Linear LR requires at least 2 epochs")
    if not (math.isfinite(start_lr) and math.isfinite(end_lr)
            and 0 < end_lr <= start_lr):
        raise ValueError("Require finite 0 < lr_end <= lr for linear decay")
    if epoch <= 0:
        return start_lr
    if epoch >= epochs - 1:
        return end_lr
    return start_lr + (end_lr - start_lr) * epoch / (epochs - 1)


def lr_policy(config):
    """Old YAML files retain the original StepLR behavior."""
    name = getattr(config, "lr_schedule", "step")
    if name == "step":
        return {"name": "step", "start_lr": config.lr,
                "step_size": 10, "gamma": 0.1}
    if name != "linear":
        raise ValueError("lr_schedule must be step or linear")
    end_lr = getattr(config, "lr_end", None)
    if end_lr is None:
        raise ValueError("Linear LR requires explicit lr_end in YAML")
    linear_epoch_lr(0, config.epochs, config.lr, end_lr)
    return {"name": "linear", "start_lr": config.lr, "end_lr": end_lr,
    ...
    for epoch in range(cfg.epochs):
        if policy["name"] == "linear":
            for group in optimizer.param_groups:
                group["lr"] = linear_epoch_lr(epoch, cfg.epochs, cfg.lr, policy["end_lr"])
        epoch_lr = optimizer.param_groups[0]["lr"]
        print("EPOCH_START epoch=%d lr=%.12g" % (epoch, epoch_lr), flush=True)
        net.train()
        total = 0.0
        memory(reset=True)
        ...
        if scheduler is not None:
            ...
            # Match existing next_lr logging; hold the endpoint after the last epoch.
            for group in optimizer.param_groups:
                group["lr"] = linear_epoch_lr(epoch + 1, cfg.epochs, cfg.lr, policy["end_lr"])
